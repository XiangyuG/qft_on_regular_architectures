...
if __name__ == '__main__':
    ...
    with open('csv_data/our_sycamore.csv', mode='w', newline='') as file:
        writer = csv.writer(file)
        # Write the header row
        writer.writerow(['num_qubits', 'Gate Count', 'swap', 'depth'])
        for i in range(4, N+1,2):
            swap_total, total_depth = main(i)
        
            # Write the collected stats for each row
            writer.writerow([i*i,0, swap_total, total_depth])
